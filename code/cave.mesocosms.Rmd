...
data <- read.table("data/cavebag/bagexp.txt", sep="\t", header=TRUE)
...
```{r}
#```{r}
png(filename="~/GitHub/Vietnam/figures/cave.bags.2.png", width = 1500, 
    height = 900, res = 96*2)
...
HC.plot <- plot(HC.means, ylim = c(0, 3), xlim = c(0.5, 3.5), 
      main = "Hoa Cương", pch = 22, bg = "white", lwd = 2, cex = 3, yaxt = "n", xaxt = "n", 
...
# Add y-label
mtext(expression('CH'[4]*' (ppm)'), side = 2, outer = TRUE, cex = 1.75, 
      line = 2, adj = 0.55)
...
MC.plot <- plot(MC.means, ylim = c(0, 3), xlim = c(0.5, 3.5), 
      main = "Minh Châu", pch = 22, bg = "white", lwd = 2, cex = 3, yaxt = "n", xaxt = "n", 
      cex.lab = 2, cex.axis = 1.5, las = 1, ylab = "", xlab = "")
      box(lwd = 2)
...
sa.cm2 <- read.table("data/surface.area.txt", sep="\t", header=TRUE)
