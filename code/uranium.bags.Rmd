...
# Radon Bag
data.Rn <- read.table("data/uranium/rn.bag.20151106.txt", sep="\t", header=TRUE)
time.Rn <- data.Rn[,1]
time.Rn <- strptime(data.Rn[,1], "%m/%d/%Y %H:%M")
time.Rn.hr <- (time.Rn - time.Rn[1])/(60*60)
CH4.Rn <- data.Rn[,2]
CH4.Rn.per <- CH4.Rn/max(CH4.Rn)
Rn.data <- as.data.frame(cbind(time.Rn.hr, CH4.Rn, CH4.Rn.per))
Rn.rate <- lm(Rn.data$CH4.Rn ~ Rn.data$time.Rn)
plot(Rn.data$time.Rn,Rn.data$CH4.Rn)
abline(Rn.rate)
...
radon.rate <-Rn.rate$coefficients[2]
...
# one-sample t-test: radon
r.t.test <- t.test(control.rates, mu = radon.rate)
```
...
```{r}
png(filename="/Users/lennonj/GitHub/radiolyticCH4/figures/lab.bags.concentration.png",
...
# Plot radon bag
points(time.Rn.hr[1:936], CH4.Rn[1:936], type = "l", 
       col = "red", lty = 3, cex = 2, lwd = 2)
...
legend("bottomright", c("- radiation", "+ uranium", "+ radon"), bty = "n", y.intersp = 1, lty = c(3, 1), 
       lwd = 2.5, seg.len = 2.25, col = c("grey","black", "red"), cex = 1)
...
library(png)
library(grid)
img <- readPNG("/Users/lennonj/GitHub/radiolyticCH4/figures/lab.bags.concentration.png")
grid.raster(img)
```
